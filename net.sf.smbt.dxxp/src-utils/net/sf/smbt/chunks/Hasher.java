...
public class Hasher {	
	...
	private static synchronized void calculateStaticHash(int daapVersion) {
		
		if (staticCalculated && (daapVersion == hashVersion))
			return;
		
		calcHash = "";
		//System.out.println("Regenerating static hash");
		
		if (daapVersion == ITUNES_45) 
			calculate45StaticHash();
		else
			calculate40StaticHash();
		
		hashVersion = daapVersion;
		
	}
	
	
	public static String GenerateHash(String url, int daapVersion/*, boolean getRequest*/) {
		int start = HASH_SIZE * 2; //request.getAccessIndex();
		int end = start + HASH_SIZE;
		calculateStaticHash(daapVersion);	
...
//			if (getRequest) {
//				nmd.Update((new Integer(3/*host.getThisRequestNumber()*/)).toString().getBytes());
