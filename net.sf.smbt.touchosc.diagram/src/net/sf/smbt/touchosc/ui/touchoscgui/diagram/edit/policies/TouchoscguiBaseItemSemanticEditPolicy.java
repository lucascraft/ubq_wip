...
import net.sf.smbt.touchosc.ui.touchoscgui.diagram.edit.helpers.TouchoscguiBaseEditHelper;
 ...
 * @generated
 */
public class TouchoscguiBaseItemSemanticEditPolicy extends SemanticEditPolicy {
	 ...
	 * @generated
	 */
	protected TouchoscguiBaseItemSemanticEditPolicy(IElementType elementType) {
		...
		if (request instanceof ReconnectRequest) {
			...
			if (view instanceof View) {
				Integer id = new Integer(
						TouchoscguiVisualIDRegistry.getVisualID((View) view));
				request.getExtendedData().put(VISUAL_ID_KEY, id);
			}
		} else if (request instanceof ChangeBoundsRequest) {
			if (((ChangeBoundsRequest) request).getEditParts() != null
					&& !((ChangeBoundsRequest) request).getEditParts()
							.isEmpty()) {
				CompoundCommand cc = new CompoundCommand();
				for (Object p : ((ChangeBoundsRequest) request).getEditParts()) {
					if (p instanceof TouchOscTabPageEditPart) {
						return super.getCommand(request);
					} else if (p instanceof ShapeEditPart) {
						cc.add(handleShapeNodeEditPartBounds((ShapeEditPart) p));
					}
				}
				return cc;
			}
	...
	private Command handleShapeNodeEditPartBounds(ShapeEditPart p) {
		EObject eObj = p.getNotationView().getElement();

		if (eObj instanceof TouchOscControl) {
			Dimension nodeSize = ((ShapeEditPart) p).getSize();
	...
	private Command getEditHelperCommand(IEditCommandRequest request,
		...
		if (editPolicyCommand != null) {
			...
			request.setParameter(TouchoscguiBaseEditHelper.EDIT_POLICY_COMMAND,
					command);
		}
		IElementType requestContextElementType = getContextElementType(request);
		request.setParameter(TouchoscguiBaseEditHelper.CONTEXT_ELEMENT_TYPE,
	...
	private IElementType getContextElementType(IEditCommandRequest request) {
		IElementType requestContextElementType = TouchoscguiElementTypes
