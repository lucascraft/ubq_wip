...
package net.sf.smbt.osc.tuio20.cmd;
...
public class Tuio20EventHandler extends AbstractQxEventHandlerImpl {
	public void handleQxEvent(Event event) {
		if (event.getKind().equals(EVENT_KIND.RX_DONE)) {
			...
			if (obj instanceof UdpChannel) {
				...
				if (event.getCmd() instanceof Tuio2B3D) {
					packet.setData(
						((Tuio2B3D)event.getCmd()).getMsg().getByteArray()
					);
	...
	private List<OSCPacket> processTuioBundle(CompoundCmd bundle) {
		return processTuioBundle(bundle, new ArrayList<OSCPacket>());
	}
	
	private List<OSCPacket> processTuioBundle(CompoundCmd bundle, List<OSCPacket> packets) {
		for (Cmd c : bundle.getChildren()) {
			if (c instanceof Tuio2B3D) {
				packets.add(Tuio11CmdUtils.INSTANCE.createOSCPacketFromCmd(c));
			} else { // TuioBunle 
				packets.addAll(processTuioBundle((CompoundCmd)c, new ArrayList<OSCPacket>()));
			}
		}
		return packets;
	}
