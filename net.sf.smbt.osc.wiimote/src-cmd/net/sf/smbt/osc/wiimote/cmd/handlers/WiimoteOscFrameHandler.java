...
public class WiimoteOscFrameHandler extends AbstractFrameInterpreterImpl {
	private OSCByteArrayToJavaConverter converter;
	public WiimoteOscFrameHandler() {
	...
	public synchronized Cmd byteArray2Cmd(final byte[] stream) {
		if (stream != null && stream.length>1) {
			...
			if (packet instanceof OSCMessage) {
				...
				Cmd cmd = WiimoteCmdUtils.INSTANCE.createWiimoteCmdFromOSCMsg((OSCMessage) packet);
				if (Platform.inDebugMode()) {
					System.out.println("OSC packet IN : " + addr );
