...
public class NXTCommand implements NXTProtocol {
	...
	/**
	 * Name of current running program. !! Leaves null character at end.
	 * UNTESTED
	 * @return
	 */
...
//		 The code below is repetative but can't use sendRequest() because SYSTEM_COMMAND being used
	...
	/**
	 * Returns the number of bytes for a command in the low-speed buffer
	 * or the high-speed buffer (0 = no command is ready).
	 * UNTESTED
	...
	public byte [] poll(byte bufferNumber, byte commandLength) {
		...
		if(reply[3] == 0) {
			...
			System.out.println("Array length is : " + reply.length + " (should be 4 less than Poll length).");
...
//		 !! Needs to check port to verify they are correct ranges.
	 ...
	 * NOTE: Normal Bluetooth commands do not keep the NXT alive. It will power
	 * off even if you have been regularly sending commands. Must use keepAlive()
	 * UNTESTED
	...
	public long keepAlive(){
		...
		nxtComm.sendData(request);
		byte [] reply = nxtComm.readData();
		int sleepLimit = (0xFF & reply[3]) | ((0xFF & reply[4]) << 8)| ((0xFF & reply[5]) << 16)| ((0xFF & reply[6]) << 24);
		return sleepLimit;
	}
	...
	public static void open() throws RuntimeException{
		...
		NXTCommFactory factory = new NXTCommFactory();
		singleton.nxtComm = factory.createInstance();
	 ...
	 * Custom leJOS NXJ command (not part of official LCP)
	 * This command only works with leJOS NXJ firmware.
	 * It will not work with the LEGO firmware.
	 * UNTESTED
	 * @return
	 */
