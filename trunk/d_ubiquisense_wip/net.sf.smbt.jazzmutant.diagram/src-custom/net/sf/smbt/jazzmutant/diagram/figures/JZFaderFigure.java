...
public class JZFaderFigure extends AbstractTextualLemurFigure {
	public static int FADER_MARGIN = 12;
	private JZFader fader;

    public JZFaderFigure(JZFader elem) {
    	super(elem);
    	fader = elem;
   }
    ...
    public void paint(Graphics graphics) {
        ...
        int dist = 0; //new Float((1-fader.getValue()) * (rect.height- 50) ).intValue();
        ...
        graphics.popState();
		   ...
		   graphics.pushState();
	    	...
	    	graphics.drawRoundRectangle(getParent().getBounds().getCopy().shrink(1, 1), 12, 12);
	       	
	    	graphics.setForegroundColor(fg);
	    	graphics.setBackgroundColor(fg);
	    	graphics.setAlpha(66);
	    	graphics.fillRoundRectangle(getParent().getBounds().getCopy().shrink(2, 2), 12, 12);

	    	rect = getBounds().getCopy().shrink(2, 2);
