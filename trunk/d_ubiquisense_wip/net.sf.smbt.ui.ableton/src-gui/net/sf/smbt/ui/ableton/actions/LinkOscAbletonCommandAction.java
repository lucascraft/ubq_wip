...
public class LinkOscAbletonCommandAction extends AbstractLinkCommandAction {
	private UbiKnob knob;
	@SuppressWarnings("unused")
	private String oscTag;
	private float factor;
	public LinkOscAbletonCommandAction(UbiKnob knob, String oscTag) {
		this(knob, oscTag, 1f);
	}
	public LinkOscAbletonCommandAction(UbiKnob knob, String oscTag, float factor) {
		super("Link OSC");
		this.knob = knob;
		this.oscTag = oscTag;
		this.factor = factor;
				...
				if (o instanceof EZVar) {
					...
					if (oscTag != null && oscTag.startsWith("/")) {
						QuanticMojo.INSTANCE.subscribe("osc",
							oscTag, 
							new LinkFeedbackAbletonQxEventHandler(
								knob, 
								oscTag, 
								idx, 
								factor
